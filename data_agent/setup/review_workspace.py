...
from data_agent.setup.review import (
...
GENERATED_ROOT = (ROOT / "workspaces/models").resolve()
...
def render_review_html(state: dict[str, Any], *, token: str = "") -> str:
    from data_agent.setup.review_ui import render_review_html_document
...
def _restricted_generated_path(value: str | Path) -> Path:
    ...
    if path != GENERATED_ROOT and GENERATED_ROOT not in path.parents:
        raise ContractError("review artifacts must remain under workspaces/models")
    return path
