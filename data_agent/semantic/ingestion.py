...
def _extension(data: dict[str, Any]) -> dict[str, str]:
    return {"vendor_name": EXTENSION_VENDOR, "data": json.dumps(data, sort_keys=True)}
...
def _expression(value: str, dialect: str = "SNOWFLAKE") -> dict[str, Any]:
    return {"dialects": [{"dialect": dialect, "expression": value}]}


def build_osi_from_ir(ir: dict[str, Any], model_name: str | None = None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for source_dataset in source_datasets:
        ...
        for source_field in source_dataset.get("fields", []):
            ...
            expression = str(normalized or f"{dataset_name}.{field_name}")
            translation_state = _state(source_field.get("translation_status", "exact" if normalized else "equivalent-with-assumptions"))
            field: dict[str, Any] = {
                "name": field_name,
                "expression": _expression(expression, str(source_field.get("dialect") or "SNOWFLAKE")),
                "custom_extensions": [
                    _extension(
                        {
            ...
            synonyms = source_field.get("synonyms")
            if isinstance(synonyms, list) and synonyms:
                field["ai_context"] = {"synonyms": [str(item) for item in synonyms]}
            fields.append(field)
                ...
                _extension(
                    {
                        ...
                        "translation_status": _state(source_dataset.get("translation_status", "exact")),
                    }
                )
            ],
        }
                    ...
                    _extension(
                        {
                            ...
                            "translation_status": _state(source_relationship.get("translation_status", "exact")),
                        }
                    )
                ],
            }
    ...
    for source_metric in ir.get("metrics", []):
        ...
        metric: dict[str, Any] = {
            "name": metric_name,
            "expression": _expression(str(normalized), str(source_metric.get("dialect") or "SNOWFLAKE")),
            "custom_extensions": [
                _extension(
                    {
        ...
        synonyms = source_metric.get("synonyms")
        if isinstance(synonyms, list) and synonyms:
            metric["ai_context"] = {"synonyms": [str(item) for item in synonyms]}
        ...
        issues.append(
            {
                ...
                "element": str(unsupported.get("field") or unsupported.get("source_file") or "source"),
