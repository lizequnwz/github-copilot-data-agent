...
`examples/semantic-setup/tableau/world.tds` as the working example. A `.twb` workbook is also
supported when the workbook is the available source. A `.tde` is binary and must have a same-named
`.tds` sibling or an explicit `--descriptor`.
