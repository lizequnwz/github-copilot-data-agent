...
from data_agent.tools.snowflake import connection_check
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Validate configuration and test a Snowflake connection in one command."
    )
    result.add_argument("--config-path", default="snowflake_config.yaml")
    return result


def main(argv: list[str] | None = None) -> int:
    args = parser().parse_args(argv)
    exit_code, lines = run_check(args.config_path)
