...
def render_chart(request: dict[str, Any]) -> dict[str, Any]:
    ...
    alt_text = str(
        spec.get("alt_text")
        or f"{title}. Values range from {_number(min(values))} to {_number(max(values))}{unit}."
    )
    ...
    parts = [
        ...
        '<g font-family="system-ui,-apple-system,sans-serif" fill="#334155">',
    ]
    for tick in range(5):
        value = low + (span * tick / 4)
        y = top + plot_h - (tick / 4) * plot_h
        parts.append(
            f'<line x1="{left}" x2="{width-right}" y1="{y:.1f}" y2="{y:.1f}" stroke="#dbeafe" stroke-width="1"/>'
        )
        parts.append(
            f'<text x="{left-12}" y="{y+4:.1f}" text-anchor="end" font-size="12">{html.escape(_number(value))}</text>'
        )
    zero_y = top + ((high - 0.0) / span) * plot_h
    parts.append(
        f'<line x1="{left}" x2="{width-right}" y1="{zero_y:.1f}" y2="{zero_y:.1f}" stroke="#64748b" stroke-width="1.5"/>'
    )
    if unit:
        parts.append(
    ...
    if spec["type"] == "bar":
        for index, (label, value) in enumerate(zip(labels, values)):
            ...
            parts.append(
                f'<rect x="{x:.1f}" y="{bar_top:.1f}" width="{slot*0.68:.1f}" height="{bar_h:.1f}" rx="4" fill="#1e40af"/>'
            )
            label_y = max(bar_top - 8, 16) if value >= 0 else min(bar_top + bar_h + 16, height - bottom + 24)
            parts.append(
                f'<text x="{x+slot*0.34:.1f}" y="{label_y:.1f}" text-anchor="middle" font-size="12" font-weight="600">{html.escape(_number(value))}</text>'
            )
    else:
        ...
        for (x, y), value in zip(points, values):
            parts.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="5" fill="#1e40af" stroke="#fff" stroke-width="2"/>')
            parts.append(
                f'<text x="{x:.1f}" y="{max(y-12, 18):.1f}" text-anchor="middle" font-size="12" font-weight="600">{html.escape(_number(value))}</text>'
            )
...
def render_report(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(validation, dict) or validation.get("status") != "pass":
        raise ContractError("a passing result-validation response is required")
    required = [
        "title",
        "source_tier",
        "semantic_model",
    ...
    table_rows = "".join(
        "<tr>"
        + "".join(
            f'<td class="{"number" if isinstance(value, (int, float)) else ""}">{html.escape("—" if value is None else str(value))}</td>'
            for value in row
        )
        + "</tr>"
        for row in rows[:500]
...
<style>
:root{{--background:#f8fafc;--surface:#fff;--surface-muted:#e9eef6;--text:#172554;--muted:#475569;--primary:#1e40af;--accent:#b45309;--border:#bfdbfe;--ring:#1e40af;--success:#166534;--shadow:0 12px 30px rgba(30,64,175,.08)}}
*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:var(--background);color:var(--text);font:16px/1.6 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}}a{{color:var(--primary)}}a:focus-visible,summary:focus-visible{{outline:3px solid var(--ring);outline-offset:3px}}.skip{{position:absolute;left:-9999px;top:1rem;background:var(--surface);padding:.75rem 1rem;z-index:10}}.skip:focus{{left:1rem}}main{{width:min(1160px,calc(100% - 2rem));margin:0 auto;padding:3rem 0 5rem}}header{{display:grid;gap:1rem;margin-bottom:2rem}}.eyebrow{{color:var(--primary);font-size:.8rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase}}h1{{font-size:clamp(2rem,5vw,4rem);line-height:1.02;letter-spacing:-.04em;margin:0;max-width:18ch}}h2{{font-size:1.35rem;line-height:1.25;margin:0 0 1rem}}p{{max-width:72ch}}.badges{{display:flex;flex-wrap:wrap;gap:.5rem}}.badge{{display:inline-flex;align-items:center;min-height:32px;padding:.25rem .7rem;border:1px solid var(--border);border-radius:999px;background:var(--surface);font-size:.82rem;font-weight:700}}.badge.success{{color:var(--success)}}.grid{{display:grid;grid-template-columns:repeat(12,1fr);gap:1rem}}.card{{grid-column:span 12;background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:clamp(1rem,3vw,1.5rem);box-shadow:var(--shadow)}}.summary{{font-size:clamp(1.05rem,2vw,1.25rem);border-left:5px solid var(--accent)}}.chart svg{{display:block;width:100%;height:auto}}.table-wrap{{overflow:auto;border:1px solid var(--border);border-radius:10px}}table{{border-collapse:collapse;width:100%;min-width:560px}}th,td{{padding:.7rem .8rem;text-align:left;border-bottom:1px solid var(--border);vertical-align:top}}th{{position:sticky;top:0;background:var(--surface-muted);font-size:.78rem;letter-spacing:.05em;text-transform:uppercase}}tbody tr:nth-child(even){{background:color-mix(in srgb,var(--surface-muted) 40%,transparent)}}td.number{{font-variant-numeric:tabular-nums;text-align:right}}dl{{margin:0}}.definition,.meta-item{{display:grid;gap:.2rem;padding:.7rem 0;border-bottom:1px solid var(--border)}}dt{{font-weight:800}}dd{{margin:0;color:var(--muted);overflow-wrap:anywhere}}ul{{padding-left:1.25rem}}details{{border:1px solid var(--border);border-radius:10px;padding:.8rem 1rem}}summary{{cursor:pointer;min-height:44px;display:flex;align-items:center;font-weight:800}}pre{{white-space:pre-wrap;overflow-wrap:anywhere;background:#0f172a;color:#e2e8f0;padding:1rem;border-radius:8px;overflow:auto;font:13px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace}}.footer{{margin-top:2rem;color:var(--muted);font-size:.85rem}}
...
</style></head><body><a class="skip" href="#content">Skip to report</a><main id="content"><header><div class="eyebrow">Validated analytics report</div><h1>{title}</h1><div class="badges"><span class="badge success">Validation passed</span><span class="badge">{html.escape(str(metadata['confidence']))} confidence</span><span class="badge">Fresh {html.escape(str(metadata['data_freshness']))}</span></div></header><div class="grid"><section class="card summary" aria-labelledby="summary-title"><h2 id="summary-title">Executive summary</h2><p>{summary}</p></section>{chart}<section class="card" aria-labelledby="results-title"><h2 id="results-title">Validated results</h2><div class="table-wrap" tabindex="0" aria-label="Scrollable result table"><table><thead><tr>{table_head}</tr></thead><tbody>{table_rows}</tbody></table></div></section><section class="card span-7" aria-labelledby="definitions-title"><h2 id="definitions-title">Definitions</h2><dl>{definitions_html}</dl></section><section class="card span-5" aria-labelledby="method-title"><h2 id="method-title">Methodology</h2><p>{methodology}</p></section><section class="card span-6" aria-labelledby="caveats-title"><h2 id="caveats-title">Caveats</h2><ul>{caveats_html}</ul></section><section class="card span-6" aria-labelledby="provenance-title"><h2 id="provenance-title">Provenance</h2><dl>{provenance}</dl></section><section class="card" aria-labelledby="sql-title"><h2 id="sql-title">Reproducibility</h2><details><summary>Show SQL appendix</summary><pre><code>{sql}</code></pre></details></section></div><p class="footer">Generated as a self-contained HTML artifact with no remote scripts, fonts, or assets.</p></main></body></html>"""
